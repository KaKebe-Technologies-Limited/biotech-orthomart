      ...
      <div className="container mx-auto px-4 py-2 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-3">
            {socialLinks.map(s => (
              <a key={s.name} href={s.url} target="_blank" rel="noopener noreferrer" className="hover:text-brand-gold transition-colors" aria-label={s.name}>
                {s.icon}
              </a>
            ))}
          </div>
          <div className="relative group">
            ...
            <div className="absolute right-0 top-full mt-1 bg-white text-brand-dark rounded shadow-lg py-2 w-36 hidden group-hover:block z-60">
              ...
              <Link href="/contact" className="block px-4 py-1.5 text-xs hover:bg-brand-surface">Arua</Link>
            </div>
          </div>
        </div>
      </div>
